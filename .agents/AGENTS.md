...
# PrepVerse Contribution Rules

When contributing to PrepVerse:
1. **Branching**: Create a branch from develop (eature/issue-title or ix/issue-number-bug-name).
2. **Code Guidelines**: Fully typed TypeScript (no ny), utility Tailwind classes, modular functional components.
3. **PR Process**: Submit PR to the develop branch, keep PRs focused, use the PR template, and run 
pm run lint and 
pm test locally.
